...
get_source() {
	echo ">>> Clone v2fly/v2fly-core repo..."
	git clone https://github.com/v2fly/v2fly-core.git
	cd v2fly-core
	go mod download
}
...
packzip() {
	...
	cd "$TMP"
	local PKG=${SRCDIR}/v2fly-custom-${GOARCH}-${GOOS}-${PKGSUFFIX}${NOW}.zip
	zip -r "$PKG" .
...
packtgz() {
	...
	cd "$TMP"
	local PKG=${SRCDIR}/v2fly-custom-${GOARCH}-${GOOS}-${PKGSUFFIX}${NOW}.tar.gz
	tar cvfz "$PKG" .
	echo ">>> Generated: $(basename "$PKG") at $(dirname "$PKG")"
}
